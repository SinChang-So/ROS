...
#define PWM_FREQUENCY               486.959f                    /* Frequency of the PWM signal in Hz                    */
...
void Set_GptPWM_Duty(double pwm_duty)
{
    double dutyUpTime = (fTimer * (pwm_duty / 100.0f)) / PWM_FREQUENCY;
    double dutyDownTime = (fTimer * (1 - (pwm_duty / 100.0f))) / PWM_FREQUENCY;
    IfxGpt12_T2_setTimerValue(&MODULE_GPT120, dutyDownTime);
    IfxGpt12_T4_setTimerValue(&MODULE_GPT120, dutyUpTime);
}
