...
double error = 0.0, error_delta = 0.0, old_error = 0.0;
double dis = 0.0;
double output = 0.0;
...
void odomCallback(const nav_msgs::Odometry& msg)
{
	...
	dis =sqrt(pow(data.pose.pose.position.x,2)+pow(data.pose.pose.position.y,2));
	error = SET_POS - dis;
	error_delta = error - old_error;
	old_error = error;
	
	P = kp*error;
	I += ki*error*0.02;
	D = kd*error_delta/0.02;
	output = P+I+D;
	
	ROS_INFO("output = [%6.3lf]   dis = [%6.3lf]",output, dis);
	ROS_INFO("P = [%6.3lf]  I = [%6.3lf]  D = [%6.3lf]",P,I,D);
