...
#define L298_IN1  &MODULE_P00,9
#define L298_IN2  &MODULE_P00,10
...
int generate_PWM_signal(void)
{
    ...
    pwm = g_generatedPwmDutyCycle / 100 * 255;
    return pwm;
}
int generate_PWM_signal2(int map_pwm)
{
    /* Calculate the total time between two rising edges for the specific frequency */
    uint32 targetWaitTime_us = IfxStm_getTicksFromMicroseconds(BSP_DEFAULT_TIMER, (1 / g_generatedPwmFreq_Hz) * SEC_TO_USEC);

    g_calculatedPwmDutyCycle = (float)map_pwm / 255 * 100;
