...
#define SONAR_TRIGER                &MODULE_P33, 7
...
#define RELOAD_VALUE_100ms           9766u
...
void GPT12_T2_Int0_Handler(void)
{
    ...
    if(type_edge ==1 ){
        ...
        else
        {
            ...
            HIGH_T =(float32)eleps_cnt_val*40.0;
            distance = (340.0*HIGH_T)/(1000000.0*2.0);
...
void init_GPT12_module(void)
{
    ...
    IfxPort_setPinMode(SONAR_TRIGER, IfxPort_Mode_outputPushPullGeneral);
...
void SET_TRIGER(void)
{
    ...
    sint32 targetWaitTime_99ms = IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, 99);
    /* Set the pin high to trigger an interrupt */
    IfxPort_setPinState(SONAR_TRIGER, IfxPort_State_high);
    ...
    IfxPort_setPinState(SONAR_TRIGER, IfxPort_State_low);
    wait(targetWaitTime_99ms);
    /* Wait for an amount of CPU ticks that represent the calculated microseconds considering the duty cycle */
    //wait( targetWaitTime_us);
}
